...
def process_response(response, hash_suffix):
    ...
    for item in response.text.splitlines():
        ...
        if item.startswith(hash_suffix):
            return item
...
def main():
    ...
    hash = hashlib.sha1(str.encode(password)).hexdigest().upper()
    ...
    print(f'Hash = {hash}')
    if item:
        print(f'Password has been leaked {item.split(":")[-1]} times according to the https://haveibeenpwned.com password API')
    else:
        print(f'Password has not been leaked according to the https://haveibeenpwned.com password API')
